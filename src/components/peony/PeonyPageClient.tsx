...
type TabKey = "about" | "spazio" | "team" | "community";
...
export function PeonyPageClient({ galleryImages }: Props) {
  ...
  const tabs: { key: TabKey; label: string }[] = [
    { key: "about", label: p.tabAbout },
    { key: "spazio", label: p.tabSpazio },
    { key: "team", label: p.tabTeam },
    { key: "community", label: p.tabCommunity },
  ];
  ...
  function handleTabChange(key: TabKey) {
    ...
    requestAnimationFrame(() => {
      if (contentRef.current) {
        const top = contentRef.current.getBoundingClientRect().top + window.scrollY - 8;
  ...
  return (
    <main className="min-h-screen bg-[#f4efe8] text-[#211815]">
            ...
            <article className="rounded-[8px] border border-[#8b5e4a]/20 bg-[#8b5e4a]/[0.08] p-5 md:p-7">
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {["Naka San", "Peonia", "Elemiaow"].map((label) => (
                  <span key={label} className="rounded-full border border-[#8b5e4a]/20 bg-white/45 px-3 py-1.5 text-xs font-medium text-[#8b5e4a]">
                    {label}
                  </span>
                ))}
              </div>
                ...
                {content.location.travel.map((group, index) => (
                  ...
                  >
                    <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 text-sm font-medium text-[#211815] marker:hidden">
                      ...
                      <span aria-hidden="true" className="grid h-7 w-7 shrink-0 place-items-center rounded-full border border-[#211815]/10 bg-white/45 text-[#8b5e4a] transition group-open:rotate-45">
                        +
                      </span>
                    </summary>
                    <ul className="grid gap-2 border-t border-[#211815]/10 px-4 py-3">
                      {group.items.map((item) => (
                        <li key={item} className="text-sm leading-[1.55] text-[#5f524c]">{item}</li>
                      ))}
                    </ul>
                  </details>
                ))}
      ...
      {activeTab === "team" && (
        <>
          <SectionShell
            eyebrow={p.foundersEyebrow}
            title={p.foundersTitle}
      ...
      {activeTab === "community" && (
        <>
          <SectionShell
            eyebrow={p.communityTabEyebrow}
            title={p.communityTabTitle}
          ...
          <section className="mx-auto max-w-6xl px-5 pb-8 pt-4 sm:px-6 md:pb-16 md:pt-6">
            <div className="rounded-[8px] border border-[#211815]/10 bg-[#211815] p-6 text-[#f4efe8] md:p-8">
              ...
              <div className="mt-6 flex flex-col gap-2 sm:flex-row">
                <Link
                  href="/calendario"
                  className="inline-flex justify-center rounded-full bg-[#f4efe8] px-5 py-3 text-sm font-medium text-[#211815]"
                >
                  {p.finalCtaBtn1}
                </Link>
                <Link
                  href="/come-iniziare"
                  className="inline-flex justify-center rounded-full border border-[#f4efe8]/20 px-5 py-3 text-sm font-medium text-[#f4efe8] transition hover:bg-[#f4efe8]/10"
                >
                  {p.finalCtaBtn2}
                </Link>
              </div>
...
function SectionShell({
  ...
  return (
    <section id={id} className="mx-auto max-w-6xl px-5 py-7 sm:px-6 md:py-12">
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-4">
      {cards.map((card, index) => (
        <Link
          key={card.title}
          href={COMMUNITY_HREFS[index] ?? "/calendario"}
        ...
        >
          ...
          <h3 className="mt-2 font-serif text-2xl font-medium leading-[1.08]">{card.title}</h3>
          <p className="mt-2 text-sm leading-[1.6] text-[#5f524c]">{card.text}</p>
...
function TextPanel({ eyebrow, title, text }: { eyebrow: string; title: string; text: string }) {
  return (
